...
import co.edu.icesi.taller2.model.TsscTimecontrol;
...
import co.edu.icesi.taller2.service.GameServiceImp;
...
class GameTest {
	...
	class AddGame {
		...
		public void addNullGameTestThrowsException() {
			...
			verifyZeroInteractions(gameRepo);
	...
	public void gameAddTestTopicNullThrowsException() {
		...
		when(topicRepo.findById(topic.getId())).thenReturn(null);
	...
	class AddGame2 {
		...
		public void gameAddTestWithoutStoriesThrowException() {
			...
			when(topicRepo.findById(topic2.getId())).thenReturn(Optional.of(topic2));
	...
	class EditGame {
		...
		public void setUp2() {
			...
			when(gameRepo.findById(game.getId())).thenReturn(game1);
		...
		public void gameEditTestTopicNoNull() {
			...
			verify(gameRepo,times(1)).findById(game.getId());
			verify(gameRepo,times(1)).save(game);
